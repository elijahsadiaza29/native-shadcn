...
## 1. Prerequisites (Design System)

These components rely on **NativeWind v4** (Tailwind for React Native) and a standard `cn` utility.

### The `cn` utility

Create `src/lib/utils.ts` if you don't have it:
...
Add these to your `global.css`. These colors are calibrated to match the official Shadcn UI themes.
